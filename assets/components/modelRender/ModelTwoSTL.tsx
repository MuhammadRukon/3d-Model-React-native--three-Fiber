...
export default function ModelTwoSTL(props) {
  const { onLoaded, ...restProps } = props;

  // Load the STL geometry
  const geometry = useLoader(STLLoader, require("./../../models/house3.stl"));
  ...
  return (
    <group {...restProps} dispose={null} scale={0.03} position={[0, -1, 0]}>
      <mesh
        castShadow
        receiveShadow
        geometry={geometry}
